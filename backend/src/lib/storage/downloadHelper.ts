...
//   2. Disco local (localPath)   — archivos pre-cloud
//
...
export async function downloadDocumentBuffer(doc: DownloadableDoc): Promise<Buffer> {
  ...
  // 2. Disco local — archivos pre-cloud
  if (doc.localPath) {
  ...
  throw new Error('Archivo no disponible en ningún almacenamiento configurado (R2 ni disco)');
}
